...
(for some $q$) are used to predict the closing price on the day $t$ — the concept of rolling window methodology is adopted \cite{forecast_book}.
\par
...
$T_1$ and $T_2$ breakpoints (let $T_2-T_1=M$). On this interval reside two frames of variables. 
...
$\mathbf{X}[t]$ represents data available through days $t-1, t-2, \ldots, t-9$. \footnote{We chose the value of $q$ due to few reasons constant. Firstly, variable $q$ 
would result in larger time-complexity and necessity to optimize new parameter further. Secondly, we want emphasize short-term market dynamics,
which are commonly observed in financial time series and documented in the literature \cite{forecast_book}} 

Second, univariate $Y \in \mathbb{R}^M$ accumulates values of the target variable (next closing price) to the respective rows of $\mathbf{X}$. This means, that $Y[t]$ is the value of the closing price on the day $t$.
\par
...
and $w$, which denotes the time units beyond which, we aim to determine predictions. For the purposes of this thesis and its next-day prediction methodology we define $w=1$.
...
\State \# The prediction target day is $t+p+1$, we assume we have available data up to day $t+p$
\For{each stock in database}
    \State Retrieve stock's data for features $\mathbf{X}$, target $Y$
    \For{$t = T_1$ to $T_2 - (p + 1)$}
        ...
        % Preistotu? 
        \State \# $\mathbf{X}_\text{predict}$ correspond to the latest available data 
        \State Train MODEL on $(\mathbf{X}_{\text{train}}, Y_{\text{train}})$
        \State Generate prediction: MODEL$(\mathbf{X}_{\text{predict}})$
    \EndFor
\EndFor
\end{algorithmic}
\end{algorithm}
...
The multivariate $\mathbf{X}$ contains pre-chosen shifted variables (Subchapter \ref{subchapter:2.3}). In this thesis, time interval between 15th of September 2017 and 31st of December 2022 (1333 trading days)
