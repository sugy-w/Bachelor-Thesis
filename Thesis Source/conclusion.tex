...
Subsequent analyses of the application of the models to the portfolio optimization were introduced in Chapter \ref{chap:4}. Evaluation results suggest that even though prediction of most of the models were inferior to the Naïve benchmark,
...
\newline
\par
Apart from the compared studies \cite{krauss,konferencia1,zhao}, possibilities of applying such predictions for portfolio management were researched. Selected studies focused solely on designing the prediction pipeline and prediction evaluation. Connecting this methodology with portfolio optimization increases the application value of the research.

Used ML algorithms cannot comprehend the time-flow dependency properly \cite{forecast_book}. However, by using the ARIMA model we addressed the issue. Even though time series models are used in the research quite often, simpler frameworks are used more even with inferior performance \cite{review}. This suggests a suitable alternative to the conventional ML frameworks. 

As stated earlier, prediction Naïve benchmark showed competitive results, in comparison with the other models. This shows that there exists a less computationally heavy prediction alternative, capable of outperforming advanced ML frameworks. Some studies tend to compare models between each other \cite{mei2018stock} or omit the comparison at all \cite{zhao}. The thesis shows potential
false significance of the models compared to the low computationally heavy approaches. % Tu sa bude treba pobaviť

Quantitative and qualitative comparison with the existing studies is limited. Direct comparison cannot be done, as there is no widely known study conducting similar rolling-window prediction on the whole market segment. However, several implicit conclusions can be done.
...
Apart from that, there is possibility of implementing more advanced frameworks – neural networks. Some studies \cite{review} suggest more reliable prediction results than conventional ML algorithms. However, we decided not to include neural networks in this research, as the data preparation implemented does not align with the requirements of other used frameworks. Altering the preparation process 
