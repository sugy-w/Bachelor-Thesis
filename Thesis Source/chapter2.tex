...
single stock or a very limited set of a few stocks \cite{clanok5, konferencia1}. Others apply predictive modeling methods
...
Secondly, univariate $\mathbf{Y} \in \mathbb{R}^M$ accumulates values of the target variable (next closing price) of the particular stock, to the respective rows of $\mathbf{X}$. This means, that $\mathbf{Y}_t$ (the $t$-th value in the $Y$) is the value of the closing price on day $t$.
\par
...
\For{each stock in database}
    ...
    \For{$t = T_1$ to $T_2 - (p + 1)$}
        ...
        \State $\mathbf{Y}_{\text{train}} \gets (\mathbf{Y}_t, \mathbf{Y}_{t+1}, \ldots, \mathbf{Y}_{t+p})$
...
market play a vital part in the whole financial segment, which suggests, that including such assets in the database might be sufficient.
\par
