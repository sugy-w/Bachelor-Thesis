...
next-day prediction, which means that by using trading days $t-(p-1)$, $t-(p-2)$, ..., $t$, we predict the price value of a financial instrument on the $t+1$ day,
we can use the methodology of \textbf{rolling window} or commonly named as \textbf{sliding window} \cite{forecast_book}.
\par
In this approach, we define a static time-series interval, which serves as a hybrid train-validation time interval. Let's define the overall data interval being between
$T_1$ and $T_2$ breakpoints \textit{(global interval)}. On this interval reside two frames of variables – multivariate $\mathbf{X} \in \mathbb{R}^{M \times F}$, where $F$ denotes the number of features used, which serves 
as an input variable and univariate $Y \in \mathbb{R}^M$ accumulates values of the target variable to the respective rows of $\mathbf{X}$.
\par
Further, we define parameter $1 \leq p<M$, which is the length of the training subwindow (look-back parameter)
and $w$, which denotes the time units beyond which, we aim to determine predictions. For the purposes of this thesis and its next-day prediction methodology we define $w=1$, but in the general auto-regressive
...
\State We suppose $w \gets 1$
    \For{$t = T_1+p$ to $T_2 - 1$}
    \State $\mathbf{X}_t \gets \mathbf{X}[t-p+1 : t]$
    \State $Y_t \gets Y[t-p+1 : t]$
    \State Train respective model on $(\mathbf{X}_t, Y_t)$
    \State Generate one-day-ahead forecast $\hat{Y}_{t+1}$
\EndFor
\end{algorithmic}
\end{algorithm}
...
is a single time unit. Secondly, by ensuring overlapping, we maximize the number of possible training, thus validation, data points, 
...
The multivariate $\mathbf{X}$ contains pre-chosen variables (Subchapter \ref{subchapter:2.3}). In this thesis, time interval between 15th of September 2017 and 31st of December 2022 (1333 trading days)
is used as training-validation interval and time interval between 1st of January 2023 and 16th of January 2026 (761 trading days) is used as test interval.
