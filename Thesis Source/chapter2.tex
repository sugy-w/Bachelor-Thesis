...
their possible importance in terms of predictive modeling, and lastly their subsequent processing 
into specific forms, which were used throughout all the related analyzes in this thesis.
\par
Market itself and particularly the price movement of financial instruments is a complex and
...
methods of machine learning on the particular market segment. Many papers focus on a
single stock or a very limited set of a few stocks \cite{clanok5, vijh,konferencia1}. Others apply predictive modeling methods
to stock indices (e.g., US S\&P 500 and NASDAQ Composite or German DAX) \cite{krauss, patel}; however, there is 
a small number of sources dealing with a market segment, analyzing single stocks included in it (see Introduction).
\par
...
\par
For the purposes of defining desired terminology, let us define a time interval on which we analyze an instrument's attribute movement as $[t_1, t_2]$, where
naturally $t_1 < t_2$. The terminology, could be universally defined for any time interval with various frequency.
\par
...
The visualization of the terms for a sample interval of a Microsoft stock between the beginning of June 2025 and the beginning of September 2025 is pictured in Figure \ref{fig:2.1}.
\par
...
to pay parts of their profit to their investors on a regular basis (e.g., Apple, Oracle) \cite{dividendy}. Some companies do not pay out dividends, even though the attribute was included in the database for all stocks.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{images/TermsVisual.png}
    \caption{\centering Visualization of the defined attribute's movement terms on Microsoft stock price.}
    \label{fig:2.1}
\end{figure}
...
\par
In our approach, we define a static time-series interval, which serves as a hybrid train-validation time interval. Let us define the overall data interval being between
...
which are commonly observed in financial time series and documented in the literature \cite{forecast_book}.} To ensure that the data in the $\mathbf{X}$ are not biased, each particular row is transformed via Z-score standardization.

Secondly, univariate $\mathbf{Y} \in \mathbb{R}^M$ accumulates values of the target variable (next closing price) of the particular stock, to the respective rows of $\mathbf{X}$. This means that $Y_t$ (the $t$-th value in the $Y$) is the value of the closing price on day $t$.
\par
...
between input and target matrices. Algorithm \ref{alg:sliding_window} shows what parts of the global $\mathbf{X}, \mathbf{Y}$ are directly available at the certain moment of the model building.
\begin{algorithm}[H]
...
\For{each stock in database}
    ...
    \For{$t = T_1$ to $T_2 - (p + 1)$}
        ...
        \State $\mathbf{Y}_{\text{train}} \gets (Y_t, Y_{t+1}, \ldots, Y_{t+p})$
        \State $\mathbf{X}_{\text{predict}} \gets \mathbf{X}[t+p+1]$ \>\> \# Z-score scaled
        \State \# $\mathbf{X}_\text{predict}$ corresponds to the latest available data 
        \State Train model $\hat{f}$ on $(\mathbf{X}_{\text{train}}, \mathbf{Y}_{\text{train}})$: so $\mathbf{Y}_{\text{train}}\approx\hat{\mathbf{Y}}_{\text{train}}=\hat{f}(\mathbf{X}_{\text{train}})$ 
...
The multivariate $\mathbf{X}$ contains pre-chosen shifted variables (see Section \ref{subchapter:2.3}). In this thesis, time interval between September 15, 2017 and December 31, 2022 (1333 trading days)
...
for machine learning purposes wisely. In this section, we aim to show and justify the 
process of our general feature selection, mostly based on \textit{a priori} knowledge of processes within
financial markets and the variable's most likely effect on stock markets.

For all of the mentioned variables in this section and for all considered shifted historical dates (up to $q = 9$ backwards), the respective \textbf{open}, \textbf{close}, \textbf{low}, and \textbf{high} values are present in the resulting feature matrix $\mathbf{X}$.
On top of that, where appropriate, the \textbf{volume} and \textbf{dividends} variables are also included.  
...
\par
For these purposes, we decided to include two major stock indices into our database. The general index \textbf{Standard and Poor's 500}, commonly
...
comes from the fact that most of these assets play an essential part in further processing, consumption and industry. Matsumoto et al. \cite{clanok3} suggest that the commodity
market plays a vital part in the whole financial segment, which suggests that including such assets in the database might be sufficient.
\par
In this thesis, we included data regarding two major commodities – \textbf{crude oil} and \textbf{gold}. Even though there might not be a clear
...
price of every possible currency pair, thus making the acquisition of a certain amount of currency possible through various ways – currency $X$ could be
...
indicator of currency relative strength and, by that, an indicator of a particular economic competitiveness of a certain country. Taking this into account,
we assume that including such data inside our database would bring valuable information about the international competitiveness of US economy towards other major economies 
...
with respect to their overall traded volume on a daily basis, according to reports published by the Bank for International Settlements and to market conventions \cite{bis_report, forex1}.
\par
...
These bonds are further divided based on their maturity – the time within which issuer obliges to fully repay issued money.
\par
...
of possible features we included bonds with a maturity of 13 weeks, which belong to the group of short-term bonds, and bonds with 5 year long maturity. This kind of maturity
...
tracking the sentiment of investors and of the market itself. We chose to include data of \textbf{Volatility Index} (VIX), which is a specific gauge (based on S\&P 500 option data) tracking state
of the US stock market in terms of volatility and the sentiment of investors. 
\par
It presumably brings valuable information, which might not be included in all of the metrics described, regarding the \textit{a priori} mood of the market 
...
phase of repeated issuing and cancelling of international tariffs. As we can see, this political move damaged investors' exposure to the US market then.

\section{Data preparation summary}

Taking into account the process, how historical data are processed into the feature $\mathbf{X}$ matrix, it is possible to conclude the total number of features per single row.
We consider $q = 9$ shifts backwards and in total $14$ distinct financial variables. For each variable and shift, present in the database, we include in total $5$ metrics (Open, Close, High, Low and Volume).
This together yields in total $630$ features included in the feature matrix $\mathbf{X}$.

Consequently, it is suitable to state that this feature $\mathbf{X}$ is used throughout the whole interval and for all of the analyzed stocks. The key variable in the modeling process is the target vector $\mathbf{Y}$
which consists of closing prices of the particular stock (among the $65$ considered).