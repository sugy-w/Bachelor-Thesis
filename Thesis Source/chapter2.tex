...
Second, univariate $Y \in \mathbb{R}^M$ accumulates values of the target variable (next closing price) to the respective rows of $\mathbf{X}$. This means, that $Y[t]$ is the value of the closing price on day $t$.
\par
...
\For{each stock in database}
    \State Retrieve stock's data for features $\mathbf{X}$ and target $Y$
