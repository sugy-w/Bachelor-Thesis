...
single stock or a very limited set of a few stocks \cite{clanok5, konferencia1}. Others apply predictive modelling methods
...
\par
As of late 2025, the Information technology (IT) segment consists of $68$ US-traded stocks. However, we excluded three stocks (tickers APP, CRWD and DDOG), as their brief period (at most $3$ years) of trading yielded insufficient historical data for reliable modeling. In the year 2025, in terms of total capitalization, the IT group makes a total of
...
\For{each stock in database}
    ...
    \For{$t = T_1$ to $T_2 - (p + 1)$}
        ...
        \State Train model $\hat{f}$ on $(\mathbf{X}_{\text{train}}, \mathbf{Y}_{\text{train}})$: so $\hat{Y}_{\text{train}}=\hat{f}(\mathbf{X}_{\text{train}})$ 
...
The multivariate $\mathbf{X}$ contains pre-chosen shifted variables (Section \ref{subchapter:2.3}). In this thesis, time interval between September 15, 2017 and December 31, 2022 (1333 trading days)
...
\par
For these purposes, we decided to include into our database two major stock indices. The general index \textbf{Standard and Poor's 500}, commonly
...
indicator of currency relative strength and, by that, an indicator of a particular government's and economic competitiveness of a certain country. Taking this into account,
